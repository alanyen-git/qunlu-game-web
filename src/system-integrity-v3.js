/* 群陸旅誌：系統完整性修正 CURRENT-1.65.0
 * SYSTEM-INTEGRITY-3.0
 * 版本同步／舊存檔遷移閘門／跨模組稽核／玩家體驗整合／狀態修復
 */
(()=>{
  if(typeof DB!=="object"||!DB)return;
  const RELEASE="CURRENT-1.65.0",REVISION="SYSTEM-INTEGRITY-3.0",LEGACY_RUNTIME_VERSION="CURRENT-1.57.0";
  const CFG={event_limit:80,seen_limit:220,audit_interval_ms:60000,persist_after_repair:true};
  DB.meta=DB.meta||{};DB.meta.current_version=RELEASE;DB.meta.system_integrity_revision=REVISION;
  DB.system_integrity_system={version:REVISION,scope:["版本同步","存檔遷移","事件去重","NPC狀態完整性","職業／技能結構","長局玩家體驗"],
    rules:["CURRENT-1.57.0以前的存檔才執行舊runtime完整遷移；新版存檔禁止被舊常數反向降版。",
      "每次寫入本機存檔前，G.meta.version必須與正式發布版同步。",
      "NPC第一、第二階段與自主世界事件依事件ID去重，不修改事件內容與遊戲結果。",
      "職業技能正規化與被動相容層只補結構缺口，不重置技能XP與職業進度。",
      "旅程羅盤只讀已知狀態並提供資訊，不自動替玩家決定玩法或消耗回合。",
      "自檢只修復可確定的結構性問題，不刪除角色進度、不重置人物關係、不繞過資格規則。"],
    save_schema_changed:false,destructive_repairs:false};

  const parseVersion=v=>{const m=String(v||"").match(/CURRENT-(\d+)\.(\d+)\.(\d+)/);return m?[+m[1],+m[2],+m[3]]:null};
  const compareVersion=(a,b)=>{const x=parseVersion(a),y=parseVersion(b);if(!x||!y)return null;for(let i=0;i<3;i++)if(x[i]!==y[i])return x[i]>y[i]?1:-1;return 0};
  ...
  const knownNpcIds=()=>new Set((DB.regional_npc_archetypes||[]).map(x=>x.id).filter(Boolean));
  const knownQuestIds=()=>new Set([...(DB.quest_templates||[]),...(DB.shop_quests||[])].map(x=>x.id).filter(Boolean));

  function repairWorldState(){
    const g=game();if(!g?.worldState)return {changed:false,repairs:[]};let changed=false;const repairs=[];
    ...
    if(typeof globalThis.applyClassSkillPassiveCompat==="function")try{const r=globalThis.applyClassSkillPassiveCompat();if(r?.saveRepairs){changed=true;repairs.push("被動技能舊存檔相容修正")}}catch(e){}
  ...
  function runAudit(repair=true){
    ...
    add("passive_compat_revision",DB.meta?.class_skill_passive_compat_revision==="CLASS-SKILL-PASSIVE-COMPAT-1.0",DB.meta?.class_skill_passive_compat_revision);
    add("player_experience_revision",DB.meta?.player_experience_revision==="PLAYER-EXPERIENCE-1.0",DB.meta?.player_experience_revision);
    ...
    add("passive_compat_api",typeof globalThis.runClassSkillPassiveCompatAudit==="function");
    add("player_experience_api",typeof globalThis.runPlayerExperienceAudit==="function"&&typeof globalThis.openJourneyCompass==="function");
    ...
    const pc=typeof globalThis.runClassSkillPassiveCompatAudit==="function"?globalThis.runClassSkillPassiveCompatAudit():null;add("passive_compat_selfcheck",!pc||pc.pass===true,pc?.revision||null,pc?.issues?.slice(0,5).join("；")||"");
    const px=typeof globalThis.runPlayerExperienceAudit==="function"?globalThis.runPlayerExperienceAudit():null;add("player_experience_selfcheck",!px||px.pass===true,px?.revision||null,px?.issues?.slice(0,5).join("；")||"");

    const repairResult=repair?repairWorldState():{changed:false,repairs:[]};
    const result={revision:REVISION,release:RELEASE,pass:checks.every(x=>x.pass),checks,repair:repairResult,time:Date.now(),classSkill:cs,passiveCompat:pc,playerExperience:px};
  ...
  if(originalMigrateSave&&!globalThis.__SYSTEM_INTEGRITY3_MIGRATION_PATCHED){globalThis.migrateSave=function(){const g=game(),source=g?.meta?.version,cmp=compareVersion(source,LEGACY_RUNTIME_VERSION);let result;if(cmp===null||cmp<0)result=originalMigrateSave.apply(this,arguments);syncGameVersion();try{globalThis.syncClassSkillOptimizationState?.();globalThis.applyClassSkillPassiveCompat?.()}catch(e){}return result};globalThis.__SYSTEM_INTEGRITY3_MIGRATION_PATCHED=true}
  const originalPersist=typeof globalThis.persist==="function"?globalThis.persist:null;
  if(originalPersist&&!globalThis.__SYSTEM_INTEGRITY3_PERSIST_PATCHED){globalThis.persist=function(){syncGameVersion();return originalPersist.apply(this,arguments)};globalThis.__SYSTEM_INTEGRITY3_PERSIST_PATCHED=true}
  const originalCreateCharacter=typeof globalThis.createCharacter==="function"?globalThis.createCharacter:null;
  if(originalCreateCharacter&&!globalThis.__SYSTEM_INTEGRITY3_CREATE_PATCHED){globalThis.createCharacter=function(){const result=originalCreateCharacter.apply(this,arguments);syncGameVersion();try{globalThis.persist?.()}catch(e){}return result};globalThis.__SYSTEM_INTEGRITY3_CREATE_PATCHED=true}

  function openSystemIntegrityPanel(){
    const result=runAudit(true),bad=result.checks.filter(x=>!x.pass),good=result.checks.length-bad.length,esc=v=>String(v??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
    ...
    const cs=result.classSkill?`<div class="small">職業／技能：高優先${result.classSkill.high}｜中優先${result.classSkill.medium}｜觀察${result.classSkill.low}</div>`:"";
    const px=result.playerExperience?`<div class="small">玩家體驗：${result.playerExperience.pass?"通過":"需檢查"}｜32,000回合長局整合層</div>`:"";
    if(typeof showModal==="function")showModal("系統自檢",`<div class="card"><b>${esc(REVISION)}</b><br><span class="small">通過 ${good}/${result.checks.length} 項｜發布版 ${esc(RELEASE)}｜${result.pass?"核心檢查通過":"仍有需處理項目"}</span>${cs}${px}</div><div class="card"><b>異常項目</b>${badHtml}</div><div class="card"><b>本次修復</b>${repairs}</div>`)
  }
  function patchMoreMenu(){
    if(globalThis.__SYSTEM_INTEGRITY3_MENU_PATCHED||typeof globalThis.openMoreMenu!=="function")return;
    const original=globalThis.openMoreMenu;globalThis.openMoreMenu=function(){const result=original.apply(this,arguments);setTimeout(()=>{const grid=typeof document!=="undefined"?document.querySelector("#modalBody .more-grid"):null;if(grid&&!grid.querySelector("[data-system-integrity]")){const b=document.createElement("button");b.className="more-card";b.dataset.systemIntegrity="1";b.innerHTML='<span class="more-icon">✓</span><span>系統自檢</span>';b.addEventListener("click",openSystemIntegrityPanel);grid.appendChild(b)}},0);return result};globalThis.__SYSTEM_INTEGRITY3_MENU_PATCHED=true
  }
