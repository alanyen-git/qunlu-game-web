...
(async()=>{
  assert.equal(cacheName,"qunlu-pwa-v158");
  ...
  stores.set("qunlu-pwa-v157",new Map([["old",new Response("stale")]]));
  let activate;
  events.get("activate")({waitUntil(p){activate=p;}});
  await activate;
  assert(!stores.has("qunlu-pwa-v157"));

  const js=url("./src/runtime.js?v=CURRENT-2.14.5");
  ...
  const css=url("./assets/css/game.css?v=CURRENT-2.14.5");
