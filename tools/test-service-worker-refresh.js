...
(async()=>{
  assert.equal(cacheName,"qunlu-pwa-v157");
  ...
  stores.set("qunlu-pwa-v156",new Map([["old",new Response("stale")]]));
  let activate;
  events.get("activate")({waitUntil(p){activate=p;}});
  await activate;
  assert(!stores.has("qunlu-pwa-v156"));

  const js=url("./src/runtime.js?v=CURRENT-2.14.4");
  ...
  const css=url("./assets/css/game.css?v=CURRENT-2.14.4");
